...
> **文档时效性** | 归档日期：2026-05-25 | 涉及版本：版本未明确 | 当前最新：4.3.3 | 状态：⚠️ 可能过时
...
- **CMake 大版本升级**：文档编写时 CMake 处于 3.x 系列，当前已发布 4.3.3（2026-05），经历了 3.x → 4.0 → 4.3 多次大版本升级。核心 CMake 命令语法保持向后兼容，但部分策略（CMP_POLICY）和默认行为可能已变更。
- **scikit-build-core-conan**：文中使用 `scikit-build-core-conan` 作为构建后端，该项目的名称或配置方式可能已有更新，建议检查其最新文档。  
