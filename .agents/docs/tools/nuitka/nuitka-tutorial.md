...
> **文档时效性** | 归档日期：2026-05-25 | 涉及版本：0.9.6 / 2.8.4（文中版本号不一致） | 当前最新：4.1 | 状态：⚠️ 可能过时
...
- **版本跨度大**：文档编写时 Nuitka 处于 2.x 系列，当前已发布 4.1（2026-05），经历了 2.8 → 3.x → 4.0 → 4.1 多次大版本升级，部分 CLI 参数和行为可能已变更。
- **文中版本号不一致**：Windows 验证示例引用 "2.8.4"，macOS/Linux 验证示例仍引用 "0.9.6"（2022 年版本），后者明显过时。
- **Python 版本支持**：文中称支持 Python 3.4-3.13，当前 Nuitka 4.1 已支持 Python 3.14。
- **GitHub Actions 示例**：文中使用 `actions/checkout@v3`、`actions/setup-python@v4`、`actions/upload-artifact@v3`，当前推荐使用 v4 版本。
