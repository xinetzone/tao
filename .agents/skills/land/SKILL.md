...
while true; do
  gh api repos/{owner}/{repo}/issues/"$pr_number"/comments \
    --jq '.[] | select(.body | startswith("## Agent Review")) | .id' | rg -q '.' \
    && break
  sleep 10
done
...
- Agent review jobs retry on failure and are non-blocking; use the presence of
  `## Agent Review` (or `## Codex Review`) issue comments (not job status) as the
  signal that review feedback is available.
...
- Agent reviews arrive as issue comments posted by GitHub Actions. They
  start with `## Agent Review` and include the reviewer's
...
- If feedback requires changes:
  ...
  - The land watcher treats agent review issue comments as unresolved until a
    newer `[agent]` issue comment is posted acknowledging the findings.
- Only request a new agent review when you need a rerun (e.g., after new
  ...
  - Before requesting a new review, re-run the land watcher and ensure
    ...
    replies).
  - After pushing new commits, the review workflow will rerun on PR
  ...
  - Only request a new review if there is at least one new commit since the
    previous request.
  - Wait for the next review comment before merging.
