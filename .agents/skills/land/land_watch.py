...
def latest_review_request_at(comments: list[dict[str, Any]]) -> datetime | None:
    ...
    for comment in comments:
        if is_agent_bot_user(comment.get("user", {})):
            continue
        body = comment.get("body") or ""
        if "@agent review" not in body and "@codex review" not in body:
            continue
...
def filter_agent_comments(
    comments: list[dict[str, Any]],
    review_requested_at: datetime | None,
) -> list[dict[str, Any]]:
    latest_agent_reply = latest_agent_reply_by_thread(comments)
    latest_issue_ack = latest_agent_issue_reply_time(comments)
    agent_comments = [c for c in comments if is_agent_bot_user(c.get("user", {}))]
...
def is_agent_bot_user(user: dict[str, Any]) -> bool:
    login = user.get("login") or ""
    return login in AGENT_BOTS


def is_bot_user(user: dict[str, Any]) -> bool:
    login = user.get("login") or ""
    if is_agent_bot_user(user):
        return True
...
def is_agent_reply_body(body: str) -> bool:
    return body.startswith("[agent]") or body.startswith("[codex]")


def is_agent_review_body(body: str) -> bool:
    return body.startswith("## Agent Review") or body.startswith("## Codex Review")


def latest_agent_issue_reply_time(
    comments: list[dict[str, Any]],
) -> datetime | None:
    latest: datetime | None = None
    for comment in comments:
...
def filter_human_issue_comments(comments: list[dict[str, Any]]) -> list[dict[str, Any]]:
    latest_ack = latest_agent_issue_reply_time(comments)
    filtered: list[dict[str, Any]] = []
    for comment in comments:
        ...
        if is_agent_review_body(body):
            continue
        if "@agent review" in body or "@codex review" in body:
            continue
        created_time = comment_time(comment)
        if (
...
def filter_agent_review_issue_comments(
    comments: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    latest_ack = latest_agent_issue_reply_time(comments)
    filtered: list[dict[str, Any]] = []
...
def is_blocking_review(
    ...
    if user_login in AGENT_BOTS:
        return state == "CHANGES_REQUESTED"
    if body.startswith("[agent]") or body.startswith("[codex]") or state in ("APPROVED", "DISMISSED"):
        return False
    blocking = False
...
def raise_on_human_feedback(
    ...
    agent_review_comments = filter_agent_review_issue_comments(issue_comments)
    human_review_comments = filter_human_review_comments(review_comments)
    if human_issue_comments or human_review_comments or agent_review_comments:
...
async def wait_for_agent_review(pr_number: int, checks_done: asyncio.Event) -> None:
    ...
    while True:
        ...
        bot_issue_comments = filter_agent_comments(issue_comments, review_request_at)
        bot_review_comments = filter_agent_comments(review_comments, review_request_at)
        ...
        if bot_comments:
            ...
            if body:
                print("Agent left comments. Address feedback before merge.")
                print(body)
                raise SystemExit(2)
...
async def watch_pr() -> None:
    ...
    agent_task = asyncio.create_task(wait_for_agent_review(pr.number, checks_done))
    ...
    success_task = asyncio.gather(agent_task, checks_task)
