...
<cite>
**本文档引用的文件**
- [routes.py](file://src/taolib/symphony/server/routes.py)
- [app.py](file://src/taolib/symphony/server/app.py)
- [dashboard.py](file://src/taolib/symphony/server/dashboard.py)
- [schema.py](file://src/taolib/symphony/config/schema.py)
- [engine.py](file://src/taolib/symphony/orchestrator/engine.py)
- [state.py](file://src/taolib/symphony/orchestrator/state.py)
- [snapshot.py](file://src/taolib/symphony/observability/snapshot.py)
- [metrics.py](file://src/taolib/symphony/observability/metrics.py)
...
## 更新摘要
**变更内容**
- 新增Symphony编排系统API模块，包含系统状态查询、问题详情获取和即时轮询触发接口
- 添加配置管理API，支持完整的配置生命周期管理
- 新增观察性监控API，提供令牌核算、运行时统计和系统快照生成功能
- 扩展项目架构图，展示Symphony编排系统的集成位置
- 更新API版本控制机制，统一采用/api/v1前缀
...
## 简介
本文件为FlexLoop项目的API参考文档，涵盖认证、配置管理、文件存储、数据同步、OAuth账户连接、任务队列、二维码生成、速率限制、邮件服务、分析服务和Symphony编排系统等模块的完整接口规范。文档遵循RESTful设计原则，采用版本控制（/api/v1），提供详细的HTTP方法、URL模式、请求/响应格式、认证方式、参数说明、返回值定义和错误处理策略。

**更新** 新增Symphony编排系统的完整API规范，包括系统状态监控、问题跟踪、令牌核算和实时仪表板功能。
...
- 任务队列：任务管理、统计、健康检查
- **Symphony编排系统**：系统状态监控、问题跟踪、即时轮询、仪表板
- **观察性监控**：令牌核算、运行时统计、系统快照
...
subgraph "Symphony编排系统"
SYM_STATE["/state<br/>系统状态"]
SYM_ISSUE["/{issue_identifier}<br/>问题详情"]
SYM_REFRESH["/refresh<br/>即时轮询"]
SYM_DASHBOARD["/dashboard<br/>监控仪表板"]
end
subgraph "观察性监控"
OBS_SNAPSHOT["系统快照"]
OBS_METRICS["令牌核算"]
...
V1 --> SYM_REFRESH
V1 --> SYM_DASHBOARD
SYM_STATE --> OBS_SNAPSHOT
SYM_ISSUE --> OBS_METRICS
SYM_REFRESH --> OBS_TOKENS
V1 --> TQ_TASKS
V1 --> QR_GENERATE
V1 --> RL_RATE
V1 --> ES_EMAIL
...
**图表来源**
- [routes.py:15-179](file://src/taolib/symphony/server/routes.py#L15-L179)
- [app.py:32-70](file://src/taolib/symphony/server/app.py#L32-L70)
- [dashboard.py:9-435](file://src/taolib/symphony/server/dashboard.py#L9-L435)
...
**更新** 新增Symphony编排系统，提供完整的系统状态监控和问题跟踪功能。
...
## 架构概览
FlexLoop采用分层架构设计，确保各模块间的松耦合和高内聚。新增的Symphony编排系统提供实时监控和问题跟踪能力。

```mermaid
graph TB
subgraph "客户端层"
CLIENT["客户端应用<br/>Web/移动端/SDK"]
ENDPOINT["API端点<br/>/api/v1/*"]
...
OBSERVABILITY["观察性监控<br/>快照/指标"]
end
subgraph "数据存储层"
MONGO["MongoDB<br/>用户/配置/日志"]
REDIS["Redis<br/>缓存/会话"]
STORAGE["文件存储<br/>本地/S3"]
TRACKER["问题跟踪器<br/>Linear API"]
WORKSPACE["工作区存储<br/>本地文件系统"]
end
subgraph "编排系统层"
ORCHESTRATOR["编排引擎<br/>状态管理"]
SNAPSHOT["快照生成器<br/>系统状态"]
METRICS["指标收集器<br/>令牌统计"]
end
CLIENT --> ENDPOINT
DASHBOARD --> ENDPOINT
ENDPOINT --> AUTH
...
- [engine.py:47-64](file://src/taolib/symphony/orchestrator/engine.py#L47-L64)
- [snapshot.py:85-153](file://src/taolib/symphony/observability/snapshot.py#L85-L153)
...
sequenceDiagram
participant Client as "客户端"
participant StateAPI as "状态API"
participant Orchestrator as "编排器"
participant SnapshotGen as "快照生成器"
Client->>StateAPI : GET /api/v1/state
StateAPI->>Orchestrator : 获取运行时状态
Orchestrator-->>StateAPI : OrchestratorState
StateAPI->>SnapshotGen : 生成系统快照
SnapshotGen-->>StateAPI : SystemSnapshot
StateAPI-->>Client : 系统状态快照
Note over Client,Orchestrator : 支持轮询间隔监控
Note over Client,Orchestrator : 实时令牌统计
```

**图表来源**
- [routes.py:18-55](file://src/taolib/symphony/server/routes.py#L18-L55)
- [snapshot.py:92-153](file://src/taolib/symphony/observability/snapshot.py#L92-L153)
...
- **包含字段**:
  ...
  - poll_check_in_progress: 轮询状态
  - next_poll_due_at_ms: 下次轮询时间
  - generated_at: 快照生成时间
- **错误码**:
  - 503: Orchestrator不可用
  - 504: 快照超时

##### 问题详情端点 (/api/v1/{issue_identifier})
- **方法**: GET
- **认证**: 需要JWT
- **路径参数**:
  - issue_identifier (string): 问题标识符（如"PROJ-123"）
- **响应**: 问题详情对象
- **包含字段**:
  - issue_identifier: 问题标识符
  - issue_id: 跟踪器内部ID
  - status: 当前状态（running/retrying/unknown）
  - running: 运行中详情（可选）
  - retry: 重试详情（可选）
- **错误码**:
  - 404: 问题不存在
...
##### 即时轮询端点 (/api/v1/refresh)
- **方法**: POST
- **认证**: 需要JWT
- **响应**: {status: string, requested_at: string}
- **状态码**: 202 Accepted
- **错误码**:
  - 503: Orchestrator不可用
...
- **认证**: 需要JWT
- **响应**: HTML仪表板页面
- **功能**: 实时监控编排状态，支持手动触发轮询

**章节来源**
- [routes.py:18-179](file://src/taolib/symphony/server/routes.py#L18-L179)
- [dashboard.py:9-435](file://src/taolib/symphony/server/dashboard.py#L9-L435)

### 观察性监控API模块
提供令牌核算、运行时统计和系统快照生成功能。
...
flowchart TD
subgraph "令牌核算"
TOKENS["TokenUsage<br/>输入/输出/总计"]
METRICS["MetricsCollector<br/>聚合统计"]
ENDPOINT["/api/v1/state<br/>系统状态"]
end
subgraph "系统快照"
SNAPSHOT["SnapshotGenerator<br/>状态投影"]
RUNNING["RunningEntry<br/>运行中条目"]
RETRY["RetryEntry<br/>重试条目"]
end
TOKENS --> METRICS
METRICS --> ENDPOINT
SNAPSHOT --> RUNNING
...
**图表来源**
- [metrics.py:29-130](file://src/taolib/symphony/observability/metrics.py#L29-L130)
- [snapshot.py:85-193](file://src/taolib/symphony/observability/snapshot.py#L85-L193)
...
- **响应字段**:
  ...
  - retrying: [RetryEntry...]
  - completed_count: int
  - rate_limits: dict | null
- **功能**: 生成编排器运行时状态的完整快照

##### 运行时统计端点 (/api/v1/state)
- **响应字段**:
  - poll_check_in_progress: boolean
  - next_poll_due_at_ms: float | null
...
## 依赖分析
各模块间存在明确的依赖关系和接口契约。新增的Symphony编排系统与观察性监控模块紧密集成。
...
subgraph "Symphony编排依赖"
SYM_DEP["编排依赖<br/>Orchestrator<br/>SnapshotGenerator<br/>MetricsCollector"]
end
AUTH_DEP --> CONFIG_DEP
AUTH_DEP --> FILE_DEP
AUTH_DEP --> SYNC_DEP
...
- [engine.py:24-34](file://src/taolib/symphony/orchestrator/engine.py#L24-L34)
- [snapshot.py:13](file://src/taolib/symphony/observability/snapshot.py#L13-L44)
...
- **压缩传输**: 静态资源启用Gzip压缩
- ****Symphony优化**: 编排器使用单线程事件循环避免状态竞争，支持即时轮询触发
...
- **同步作业异常**: 查看作业日志和失败记录
- ****Symphony编排失败**: 检查编排器状态，确认轮询间隔和并发限制配置
...
## 结论
FlexLoop提供了完整的API生态系统，涵盖了现代应用开发的核心需求。通过模块化设计、严格的认证授权机制和完善的错误处理，确保了系统的安全性、可扩展性和易用性。新增的Symphony编排系统进一步增强了系统的可观测性和自动化能力。建议在生产环境中合理配置缓存和监控，以获得最佳的性能表现。
...
- **并发控制**: 合理设置并发数避免API限流
- **缓存策略**: 利用本地缓存减少重复请求
- ****Symphony监控**: 使用/refresh端点触发即时轮询，定期调用/state端点获取系统状态

### Symphony编排系统集成指南
- **编排器集成**: 将Orchestrator实例注入到FastAPI应用状态中
- **监控仪表板**: 通过/dashboard端点访问实时监控界面
- **配置管理**: 使用SymphonyConfig进行系统配置
- **观察性监控**: 集成SnapshotGenerator和MetricsCollector进行系统监控