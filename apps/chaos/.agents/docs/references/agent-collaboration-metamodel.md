...
本节沉淀在 AgentForge 实践中验证有效的协作模式，作为 Governance Layer 的补充指南。

### 并行 Agent 文件隔离模式

当多个 Agent 并行执行时，必须遵循以下隔离约束：

**核心规则**：

1. **文件级隔离**：每个并行 Agent 只能创建或修改其独占的文件集合，不同 Agent 的文件集合不可重叠。
2. **模块级边界**：以模块（文件夹/包）为最小隔离单元划分并行任务。
3. **集成串行化**：涉及多模块协调的集成步骤（如主入口注册、跨模块导入）必须在所有并行任务完成后串行执行。
4. **冲突预防**：若两个任务可能触及同一文件，必须选择以下策略之一：
   - (a) 合并为单一 Agent 任务
   - (b) 重新划分边界消除重叠
   - (c) 串行化执行（先后顺序）
...
- 新增多个独立模块（如 CLI 子命令、引擎组件）
- 并行编写多份规格/文档
- 并行实现互不依赖的功能

**反模式**：

- ❌ 两个 Agent 同时修改同一文件（即使修改不同行）
- ❌ 未明确文件归属就启动并行
- ❌ 在并行阶段执行集成/注册步骤

**实践验证**：World CLI 开发中常态化使用 4 Agent 并行，通过严格文件隔离实现零冲突、零回归。
