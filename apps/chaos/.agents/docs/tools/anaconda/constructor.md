...
> **文档时效性** | 归档日期：2026-05-25 | 涉及版本：3.4.0 | 当前最新：3.15.3 | 状态：⚠️ 可能过时
...
- **版本跨度**：文档基于 constructor 3.4.0 编写，当前已发布 3.15.3（2026-04），跨越多个小版本，新增了冻结环境安装、架构校验、改进的日志体验等功能。
- **conda 兼容性**：constructor 3.15.x 已适配 conda 26.x 的接口变更（如 `fetch_latest()` 返回值类型变更），旧版配置在 conda 26.x 环境下可能出现 `TypeError`。
- **Python 版本支持**：constructor 3.15.0 起已移除 Python 3.9 测试，新增 Python 3.13 支持。
