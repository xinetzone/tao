...
当删除复盘报告时，必须检查替代文件是否符合命名规范：

| 检查项 | 合规格式 | 违规示例 |
|--------|----------|----------|
| 文件名格式 | `{project}-summary-{date}.md` | `task-summary-v2.md` |
| 日期格式 | `YYYYMMDD` | `2026年6月21日` |
| 后缀 | 无模糊后缀 | `最终版`、`完整版`、`final` |

**自检锚点**：在删除或合并正式目录文件后，必须自问并显式自答：
1. 是否存在其他文档引用了即将删除的文件名？
2. 是否在替代文件中添加了别名声明？
3. 如删除的是复盘报告，替代文件是否符合命名规范？

**来源**：`tech-debt-governance-checklist.md` §2 引用链路断裂治理。`methodology-essentials.md` 被删除后，用户仍多次用旧名称引用，每次需手动定位等价文件。别名机制可在文件头部建立替代关系映射。
