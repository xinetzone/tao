...
| 失败场景 | 处理方式 | 后续操作 |
|----------|----------|----------|
| 发现未更新的引用 | ❌ 阻断删除 | 输出引用列表，等待人工处理 |
| 引用文件不存在 | ⚠️ 警告但继续 | 记录异常，继续删除 |
| 引用路径无法解析 | ❌ 阻断删除 | 输出错误详情，等待人工确认 |
| grep 命令执行失败 | ⚠️ 警告但继续 | 记录异常，建议手动检查 |
...
| 文件 | 行号 | 引用内容 |
|------|------|----------|
| `docs/tech/index.md` | L45 | `[methodology](methodology-essentials.md)` |
| `.agents/docs/README.md` | L12 | `详见 methodology-essentials.md` |
...
1. **前置阻断 > 事后回滚**：在删除前检查并阻止，避免需要回滚
2. **明确告警**：输出详细的失败信息和待处理项
3. **人工确认**：关键失败场景需人工介入，不自动跳过
4. **可恢复**：删除操作可随时暂停和恢复
