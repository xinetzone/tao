# 技术文档

## 基础规范 / 常用基准

- [标准引用表：许可证 / 商业化 / 自托管 / 锁定风险](agentforge-standard-reference-matrix)
- [研究方法模板](research-methodology-template)
- [研究决策检查清单](research-decision-checklist)

## 其他技术文档

```{toctree}
agentforge-standard-reference-matrix
research-methodology-template
research-decision-checklist
session-meta-recap
tech-debt-governance-checklist
gtm-strategy-playbook
landing-page-zero-barrier-signup
optimization-patterns
optimization-patterns-cases
kimi-webbridge-notes
docker-ci-troubleshooting-patterns
research-plan-agentforge-competitor-analysis
sync-summary-agentforge-competitor-analysis
team-sync-agentforge-competitor-analysis
```

