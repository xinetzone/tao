...
try:
    ...
    release = os.environ.get(
        "TAOLIB_VERSION", "0.0.0"
    )  # 可通过 TAOLIB_VERSION 覆盖显示版本
...
TIPPY_LOCAL_SRC = (
    ROOT.parent / "doc" / "tests" / "sphinx-tippy" / "src"
)  # 本地扩展源码目录
if TIPPY_LOCAL_SRC.exists():
    # 将本地扩展源码路径插入到 sys.path 前部，确保优先导入
    sys.path.insert(
        0, str(TIPPY_LOCAL_SRC)
    )  # 修改模块搜索路径（只改变导入优先级，不改变功能）
...
def _has(mod: str) -> bool:
    ...
    try:
        return (
            _ilut.find_spec(mod) is not None
        )  # 能找到 spec 即表示模块可导入（通常已安装）
...
core_exts = [
    ...
    # API文档与站点管理
    "autoapi.extension",  # 自动从源码生成 API 文档
]
...
html_css_files = [
    ...
    "mermaid.css",
    "local.css",
]  # 额外加载的 CSS 文件：位于 html_static_path 下
...
html_copy_source = False  # 禁用“查看源文件”入口，减少文件数量

# ================================= 构建优化配置 =================================
# 禁用不必要的功能以减少输出体积
html_show_sourcelink = False  # 禁用源文件链接
html_show_sphinx = False  # 禁用 Sphinx 版本信息
html_show_copyright = True  # 保留版权信息

# 优化静态资源
html_compact_lists = True  # 压缩列表输出
html_compact_lists_gallery = True  # 压缩图库列表
...
extlinks = {
    "daobook": (
        ...
        "Daobook %s",
    ),  # :daobook:`path` -> https://daobook.github.io/path
    "xinetzone": (
        "https://xinetzone.github.io/%s",
        "xinetzone %s",
    ),  # :xinetzone:`path` -> https://xinetzone.github.io/path
}
...
try:
    ...
    cfg_path = (
        Path(__file__).parent / "_config.toml"
    )  # doc/_config.toml：集中存放主题/站点个性化配置
    if cfg_path.exists():
        _cfg = _tomllib.loads(cfg_path.read_text("utf-8"))  # 读取并解析 TOML 文本
        html_theme_options = _cfg.get(
            "html_theme_options", {}
        )  # 从 TOML 中提取主题选项
except Exception:
...
if os.environ.get("GITHUB_ACTIONS"):
    ...
    html_baseurl = os.environ.get(
        "SITEMAP_URL_BASE", "https://xinetzone.github.io/"
    )  # 站点根 URL
elif not os.environ.get("READTHEDOCS"):
    # 本地构建（且非 Read the Docs）：默认指向本地预览地址，且 URL 规则不加语言/版本前缀
    html_baseurl = os.environ.get(
        "SITEMAP_URL_BASE", "http://127.0.0.1:8000/"
    )  # 本地预览 baseurl
    sitemap_url_scheme = (
        "{link}"  # 本地预览通常不带 {lang}/{version} 前缀，避免链接不匹配
    )
else:
...
nitpick_ignore = [
    ...
    (
        "py:class",
        "docutils.parsers.rst.directives.body.Sidebar",
    ),  # 侧边栏类型在严格模式下可忽略
    # 异常类重复导出警告（errors.py 和 __init__.py 都导出）
    ("py:obj", "taolib.remote.RemoteConfigError"),
    ("py:obj", "taolib.remote.RemoteExecutionError"),
]
...
nb_execution_allow_errors = (
    True  # Notebook 执行是否允许错误：True 表示遇到异常仍继续构建
)
nb_execution_excludepatterns = [
    "cultivate/qi/InsightHub/maple-mono/**",  # 排除执行的路径模式：避免耗时/不稳定内容影响构建
    "cultivate/qi/InsightHub/iFlow/**",  # 排除执行的路径模式：避免第三方/实验性内容影响构建
]
...
# === 构建严格模式（可选） ===
nitpicky = os.environ.get("SPHINX_NITPICK", "").lower() in {
    "1",
    "true",
    "yes",
}  # 严格引用检查开关（环境变量控制）
