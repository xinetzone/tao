...
from taolib.data_sync.errors import SyncJobNotFoundError
...
class TestSyncOrchestrator:
    ...
    async def test_run_job_success(
        ...
        with patch(
            "taolib.data_sync.services.orchestrator.AsyncIOMotorClient"
        ) as mock_client:
    ...
    async def test_run_job_with_connection_error(
        ...
        """测试连接错误时抛出异常。"""
        from taolib.data_sync.errors import SyncConnectionError
...
class TestOrchestratorValidation:
    ...
    async def test_validator_filters_invalid_docs(self) -> None:
        """验证阶段过滤无效文档。"""
        from taolib.data_sync.pipeline.protocols import TransformContext
        from taolib.data_sync.pipeline.validator import DataValidator
...
class TestOrchestratorEvents:
    ...
    async def test_events_logged_on_success(self) -> None:
        ...
        with (
            ...
            patch("taolib.data_sync.services.orchestrator.logger") as mock_logger,
        ):
            mock_client.side_effect = [mock_source_client, mock_target_client]
