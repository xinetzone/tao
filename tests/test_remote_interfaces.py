...
class TestProbe(unittest.TestCase):
    ...
    def test_probe_run_options_timeout_and_retry(self) -> None:
        options = RemoteProbeRunOptions(timeout=30, retry=3)
        self.assertEqual(options.timeout, 30)
        self.assertEqual(options.retry, 3)
        merged = options.merged_run_kwargs()
        self.assertEqual(merged["timeout"], 30)

    def test_probe_run_options_timeout_none_by_default(self) -> None:
        options = RemoteProbeRunOptions()
        self.assertIsNone(options.timeout)
        self.assertEqual(options.retry, 0)
        merged = options.merged_run_kwargs()
        self.assertNotIn("timeout", merged)

    def test_probe_remote_with_timeout(self) -> None:
        conn = FakeConnection(conda_ok=True, probe_ok=True, host="h", user="u")
        report = probe_remote(
            {"host": "h", "user": "u"},
            connection_factory=lambda **kw: conn,
        ...
        self.assertTrue(report.conda_available)
        uname_call = next(item for item in conn.runs if "uname -a" in item[1])
        _, _, kwargs = uname_call
