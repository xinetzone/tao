...
class TestEventsEndpoint:
    ...
    def test_post_single_event(self, client: TestClient, app) -> None:
        ...
        with patch(
            "taolib.testing.analytics.server.api.events.get_analytics_service"
        ) as mock_get:
...
class TestAPIKeyAuth:
    ...
    def test_api_key_required_when_configured(self, app, mock_mongo_client) -> None:
        with patch("taolib.testing.analytics.server.api.events.settings") as mock_settings:
    ...
    def test_api_key_accepted(self, app) -> None:
        with (
            patch("taolib.testing.analytics.server.api.events.settings") as mock_settings,
            patch(
                "taolib.testing.analytics.server.api.events.get_analytics_service"
            ) as mock_get,
        ):
...
class TestAnalyticsEndpoints:
    ...
    def test_get_overview(self, client: TestClient) -> None:
        with patch(
            "taolib.testing.analytics.server.api.analytics.get_analytics_service"
        ) as mock_get:
            mock_service = MagicMock()
